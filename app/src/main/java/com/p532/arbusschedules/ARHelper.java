...
/**
 * Created by sudhanshu on 12/8/17.
 */
...
public class ARHelper {
    ...
    private static Ray getTouchRay(ARView arView, float x, float y){
        ...
        //ray start
        Vector4f startNormalized = new Vector4f(
                (2.0f * x) / currentCamWidth - 1.0f,
                1.0f - (2.0f * y) /currentCamHeight,
                -1.0f,
                1.0f);

        Vector4f startEyeRay = projectionMatrix.invert().mult(startNormalized);
    ...
    public static Boolean isNodeSelected(ARView arView, ARNode node, MotionEvent e, int objectSize) {
